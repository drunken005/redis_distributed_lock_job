...
const logger = (logDir, serviceName, monitorLogDir, daysToKeep, customizedLogger) => {
    if (!imp.access && !logDir) {
        throw new Error('Init logger error. logDir is undefined.')
    }
    if (!imp.access) {
        mkdirSync(logDir);
        log4js.configure(createConfig(logDir, serviceName, monitorLogDir, daysToKeep));
        initImp(customizedLogger);
    }
    return imp;
};
